...
static int zet017_device_wait_stop_adc(struct zet017_device* device, union zet017_packet* packet)
{
	int counter = 0;
	for (;;)
	{
...
static void zet017_device_update_adc_info(struct zet017_device* device)
{
...
static int zet017_device_start_adc_cmd(struct zet017_device* device, union zet017_packet* packet)
{
	if (device->device_info.start_adc == 1)
		return 0;
...
static int zet017_device_stop_adc_cmd(struct zet017_device* device, union zet017_packet* packet)
{
	for (;;)
	{
		...
		if (0 != zet017_device_wait_stop_adc(device, packet))
			break;
...
static int zet017_device_init(struct zet017_device* device, union zet017_packet* packet)
{
	for (;;)
	{
		...
		zet017_device_update_adc_info(device);
...
static void zet017_process_adc_dac(struct zet017_device* device, union zet017_packet* packet)
{
	...
	int r = select(nfds + 1, &rfds, device->device_info.start_dac ? &wfds : NULL, NULL, &tv);
	if (r == -1)
	{
		zet017_device_close(device);
		return;
...
static void zet017_process_command(struct zet017_device* device)
{
	...
	if (zet017_device_process_wakeup(device) == 0)
	{
		...
		case zet017_set_config:
			...
			zet017_device_update_adc_info(device);
			break;
		case zet017_start_adc:
			device->command.result = zet017_device_start_adc_cmd(device, &device->command.data);
			break;
		case zet017_stop_adc:
			device->command.result = zet017_device_stop_adc_cmd(device, &device->command.data);
			break;
		default:
			break;
		}
...
ZET017_TCP_API zet017_device_set_config(struct zet017_server* server, uint32_t number, const struct zet017_config* config)
{
	...
	mutex_lock(&device->state_mutex);
	uint16_t connected = device->state.connected;
	mutex_unlock(&device->state_mutex);
	if (!connected)
		return -2;
...
ZET017_TCP_API zet017_device_start(struct zet017_server* server, uint32_t number)
{
	...
	device->command.command = zet017_start_adc;
	device->command.result = 0;
	device->command.state = zet017_command_requested;
	zet017_device_wakeup(device);
...
ZET017_TCP_API zet017_device_stop(struct zet017_server* server, uint32_t number)
{
	...
	device->command.command = zet017_stop_adc;
	device->command.result = 0;
	device->command.state = zet017_command_requested;
	zet017_device_wakeup(device);
